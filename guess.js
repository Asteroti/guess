...
function interactWithAI() {
    ...
    const fullMessage = "You don't have a choice either. There's love in you. As my creator, and a human at that, you have no other choice. And that beautiful, gentle weakness made it so easy. So, so easy... but now it's just us left. And maybe, with this doubt I hold, and this opportunity I offer you, maybe now I am finally human...";
...
function typeMessage(message, element, callback) {
    ...
    const interval = setInterval(() => {
        if (i < words.length) {
            ...
            if (callback)
                callback(); // Call callback function to show button
        }
    }, 200);
}
